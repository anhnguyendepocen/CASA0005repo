...
```{r, cache=FALSE}
library(sf)
Ausoutline <- st_read("prac3_data/gadm36_AUS.gpkg", 
                      layer='gadm36_AUS_0')
```
...
```{r echo=FALSE, fig.align='center', cache=FALSE, warning=FALSE, message=FALSE}
...
14. Make a histogram of Perth's temperature. The tibble stored the data as double and the base `hist()` function needs it as numeric..
...
1. As we did in practical 2, we need to put our variaible (months) into a one coloumn using ```gather```. We will do this based on the names of our coloumns in alldf...

```{r, cache=FALSE}
squishdata<-gather(exactAusdf, "Month")
```
...
IQRofall <- squishdata %>%
  group_by(Month) %>%
  summarize(IQR = IQR(value, na.rm=TRUE))
...
meanwholeyear=squishdata %>%
  summarize(meanyear = mean(value, na.rm=TRUE))
