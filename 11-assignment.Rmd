...
**Coversheet:** You must have a coversheet at the start of your assignment (included within the single PDF) that includes your title, department, GitHub repository URL and any online website / instructions you have created (e.g. RPubs). There is no standard format for the cover sheet, just ensure it has all the required information. It is not included in the word count.
...
Yes and no. Make sure you select only appropriate methods for your proposed analysis and as the course focuses on R, ArcMap and QGIS we expect the analysis to be completed within them.
...
**How can I show my code / Arc or QGIS tool within the assignment**

If you want to provide reproducible code for your analysis then follow the relevant practical within the practical book. You can then provide a hyperlink to the url/repository within your PDF report. If you have made ArcMap or QGIS model you should still be able to upload it to GitHub, GoogleDirve, Dropbox or OneDrive and provide a hyperlink. However, make sure that whatever you choose to do, it is reproducible with appropriate documentation
