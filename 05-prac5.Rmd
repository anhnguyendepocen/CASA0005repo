...
This week, complete a draft of part 1 of the assignment. The methodology might only be in
draft form / initial ideas as we move to more advanced analysis in the second part of
the course.
:::
...
lbtibble <- Londonborough %>%
  st_drop_geometry() %>%
  as.tibble()

Joining<-Airbnb_count%>%
  ...
  rename(`Airbnb count` = `n.x`, 
         ...
         `Borough`=NAME,
         `Hotel count` = `n.y`) %>%
  dplyr::select(Borough, `GSS Code`, `Airbnb count`, `Hotel count`)
  
#remove the geometry for our pop up boxes to avoid
Joining_drop <- Joining %>%
  st_drop_geometry()

popairbnb <- Joining_drop %>%
  popupTable(.,zcol=c("Borough", "GSS Code", "Airbnb count"))
  
pophotels <- Joining_drop %>%
  popupTable(.,zcol=c("Borough", "GSS Code", "Hotel count"))

# set the colour palettes using our previously defined breaks
# the colour palettes are the same using the same breaks
# but different data
pal <- colorBin(palette = "YlOrRd", domain=Joining$`Airbnb count`, bins=breaks)
pal2 <- colorBin(palette = "YlOrRd", domain=Joining$`Hotel count`, bins=breaks)
...
map<- leaflet(Joining) %>%
  ...
  addPolygons(color="white", 
              ...
              popup = popairbnb,
              fillOpacity = 0.7,
              fillColor = ~pal(`Airbnb count`),
              group = "Airbnb")%>%
  
  addPolygons(fillColor = ~pal(`Hotel count`), 
              weight = 2,
              opacity = 1,
              color = "white",
              dashArray = "3",
  ...
  addLegend(pal = pal, values = ~`Hotel count`, group = c("Airbnb","Hotel"), 
