...
17. Objects in R can exist as a number of different data types. These include a **matrix**, a **vector**, a **data frame** and a **list**. For the purposes of this practical we will focus on data frames. These are the most flexible data format in R (although tibbles are now becoming popular as well --- they are just more flexible versions of data frames, for example they can have a list as a column). Data frames can be conceptualised in a similar way to a spreadsheet with data held in rows and columns. They are the most commonly used object type in R and the most straightforward to create from the two vector objects we just created.
...
This is my working directory for my book project! Now i can use this to load my data that is in my practical 2 folder just like this....
...
```{r, cache=FALSE}
EW <- geojsonio::geojson_read("https://opendata.arcgis.com/datasets/8edafbe3276d4b56aec60991cbddda50_2.geojson", what = "sp")
```

Or you can do a manual download from the [Office for National Statistics Open Geography Portal](http://geoportal.statistics.gov.uk/datasets/8edafbe3276d4b56aec60991cbddda50_2) via Download > Shapefile --- see point 7.
...
prac2truffle <- tm_shape(BoroughDataMap) +
...
```{r message=FALSE, warning=FALSE, cache=FALSE, eval=TRUE}
