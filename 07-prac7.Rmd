...
o<-drive_download("https://drive.google.com/open?id=1MV7ym_LW3Pz3MxHrk-qErN1c_nR0NWXy",
                  path="prac7_data/exampleGoogleDrive/LC08_L1TP_203023_20190513_20190521_01_T1.tar.gz", 
                  overwrite=T)
```
...
```{r, eval=F, cache=T}
listgoogledriveGZ <- list.files("prac7_data/exampleGoogleDrive",pattern="\\.gz$",
                          ignore.case=TRUE, full.names=TRUE) %>%
  untar(exdir="prac7_data/exampleGoogleDrive")
```

Alternatively instead of the pipe you could write...

```{r, eval=F, cache=T}
listgoogledriveGZ <- list.files("prac7_data/exampleGoogleDrive",pattern="\\.gz$",
                          ignore.case=TRUE, full.names=TRUE) 

untar(listgoogledriveGZ, exdir="prac7_data/exampleGoogleDrive")
```

The only difference is that our list of files (or just one file) has been passed to `untar` manually as i've used the `untar()` function like this...
...
`tarfile` is pathname of the `.tar` file and 

`exdir` is the directory to extract files to
...
library(RStoolbox)
MTL <- list.files("prac7_data/Lsatdata/",pattern="*MTL.txt", ignore.case=TRUE, full.names = TRUE)
